...
import CreateForm from './modules/CreateAcademyForm'
import UpdateForm from './modules/UpdateAcademyForm'
import { deleteAcademy, getAcademyList, lockUser } from '@/api/manage'
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          title: '拥有的班级列表',
          dataIndex: 'classIds',
          scopedSlots: { customRender: 'classIds' }
        }, {
          title: '是否锁定',
          dataIndex: 'locked',
  ...
  methods: {
    ...
    handleDelete (record) {
      deleteAcademy({ ids: [record.id] }).then(r => {
        if (r.success) {
    ...
    handleBatchDelete () {
      deleteAcademy({ ids: this.selectedRowKeys }).then(r => {
        if (r.success) {
          this.$refs.table.refresh()
          this.selectedRowKeys = []
        } else {
